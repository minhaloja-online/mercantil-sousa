...
auth.onAuthStateChanged(async (user)=>{
  ...
  if(user){
    ...
    if(doc && doc.exists){
      ...
      // Conta autenticada mas sem perfil salvo ainda (raro) — por segurança, fica pendente
      // em vez de virar admin sozinha.
      Store.perfil = { nome:user.email.split('@')[0], email:user.email, papel:'pendente', ativo:true };
      await db.collection('usuarios').doc(user.uid).set(Store.perfil).catch(()=>{});
    }
    $('#loading-screen').classList.add('hidden');
    $('#login-screen').classList.add('hidden');

    if(Store.perfil.papel === 'pendente'){
      mostrarTelaPendente();
      // Fica escutando: se o administrador liberar o acesso, entra direto,
      // sem precisar sair e logar de novo.
      pendingUnsub = db.collection('usuarios').doc(user.uid).onSnapshot(d=>{
        if(d.exists && d.data().papel !== 'pendente'){
          Store.perfil = d.data();
          if(pendingUnsub){ pendingUnsub(); pendingUnsub=null; }
          entrarNoApp();
        }
...
function mostrarTelaPendente(){
  $('#app').classList.add('hidden');
  $('#pending-email').textContent = Store.perfil.email;
...
function entrarNoApp(){
  ...
  $('#sidebar-user-info').textContent = `👤 ${Store.perfil.nome} · ${Store.perfil.papel}`;
  applyRoleVisibility();
  attachListeners();
}
...
$('#btn-create-account').addEventListener('click', async ()=>{
  ...
  if(!email || !senha || senha.length<6){
    $('#login-error').textContent='Preencha e-mail e uma senha com 6+ caracteres, depois toque em "Criar conta".';
    $('#login-error').classList.remove('hidden');
    return;
  }
  try{
    const cred = await auth.createUserWithEmailAndPassword(email, senha);
    // Só a primeiríssima conta do sistema vira administrador automaticamente.
    // Qualquer conta criada depois fica "pendente" até um admin liberar o acesso
    // em Configurações → Usuários.
    let papel = 'pendente';
    try{
      const existentes = await db.collection('usuarios').limit(1).get();
      if(existentes.empty) papel = 'admin';
    }catch(e){ /* se a checagem falhar, mantém pendente por segurança */ }
    await db.collection('usuarios').doc(cred.user.uid).set({ nome:email.split('@')[0], email, papel, ativo:true, criadoEm:new Date().toISOString() });
    toast(papel==='admin' ? 'Conta criada! Você é o administrador.' : 'Conta criada! Aguardando liberação de acesso.');
  }catch(err){
    $('#login-error').textContent = traduzErroAuth(err.code);
    $('#login-error').classList.remove('hidden');
  }
...
$('#btn-logout-pendente').addEventListener('click', logout);
...
function renderCurrentView(){
  switch(Store.currentView){
    ...
    case 'pdv': renderPdvList(); renderAllCarts(); break;
...
$all('.hub-link').forEach(btn=> btn.addEventListener('click', ()=> showView(btn.dataset.view)));
...
function openProdutoForm(id){
  ...
  $('#pf-foto').addEventListener('change', (e)=>{
    ...
    comprimirImagem(f, 700, 0.6).then(dataUrl=>{
      fotoBase64 = dataUrl;
      toast('Foto pronta ✓');
    }).catch(()=>{ toast('Não foi possível ler essa imagem. Tente outra foto.','err'); });
  });
  ...
  $('#pf-salvar').addEventListener('click', async ()=>{
    ...
    if(fotoBase64 && fotoBase64.length > 900000){ toast('Essa foto ainda ficou grande demais para salvar. Tente uma imagem mais simples.','err'); return; }
    const data = {
...
/* Reduz a foto (lado máximo + qualidade JPEG) antes de guardar em base64,
   porque cada documento do Firestore tem um limite de ~1MB — uma foto de
   câmera de celular sem compressão estoura isso fácil e trava o cadastro. */
function comprimirImagem(file, maxLado=700, qualidade=0.6){
  return new Promise((resolve,reject)=>{
    ...
    reader.onload = ()=>{
      ...
      img.onload = ()=>{
        ...
        if(width>height){ if(width>maxLado){ height=Math.round(height*maxLado/width); width=maxLado; } }
...
/* ============================================================
   ...
   Importante: isto NÃO usa openModal()/#modal-root, porque o formulário
   de produto (ou outra tela) pode já estar aberto ali — usar o mesmo
   contêiner apagaria o formulário por baixo. Por isso o leitor cria sua
   própria camada, sobreposta a tudo, e some sem afetar o que estava aberto.
   ============================================================ */
let html5QrInstance = null;
let scannerLido = false;
function abrirScannerPara(targetInputId){
  scannerLido = false;
  const overlay = el(`<div class="modal-backdrop" id="scanner-overlay" style="z-index:400; align-items:center;">
    <div class="modal-sheet center">
      <div class="modal-handle"></div>
      <div class="modal-head"><h3>Ler código de barras</h3><button class="btn-icon" id="scanner-fechar">✕</button></div>
  ...
  overlay.addEventListener('click', e=>{ if(e.target===overlay) fecharScanner(); });
  $('#scanner-fechar').addEventListener('click', fecharScanner);
  ...
  html5QrInstance.start({facingMode:"environment"}, config, (decodedText)=>{
    if(scannerLido) return; // evita disparo duplicado antes da câmera parar
    scannerLido = true;
...
$('#btn-venda-avulsa').addEventListener('click', ()=>{
  ...
  $('#av-add').addEventListener('click', ()=>{
    ...
    renderAllCarts(); closeModal(); toast(nome+' adicionado ✓');
  });
});

function cartTotal(){ return Store.cart.reduce((s,c)=>s+c.qtd*c.precoUnit,0); }

/* No celular, o carrinho vive dentro de um modal (aberto pelo botão "Ver
   carrinho"). No notebook (>=900px), o mesmo carrinho fica sempre visível
   numa coluna fixa ao lado da lista de produtos (#pdv-desktop-cart). Os dois
   lugares usam a mesma marcação (classe .cart-items-target) e são
   atualizados juntos por renderAllCarts(), então nunca ficam dessincronizados. */
function renderCarrinhoModal(){
  const html = `
    <div class="modal-head"><h3>Carrinho</h3><button class="btn-icon" onclick="closeModal()">✕</button></div>
    <div id="cart-items" class="cart-items-target"></div>
    <div class="perf"></div>
    <div class="cart-total-row"><span class="lbl">Total</span><span class="val mono js-cart-total">${fmtBRL(cartTotal())}</span></div>
    <button class="btn btn-primary btn-block btn-lg js-cart-finalizar" id="btn-finalizar-venda" ${!Store.cart.length?'disabled':''}>Finalizar venda</button>`;
  openModal(html);
  renderAllCarts();
  $('#btn-finalizar-venda').addEventListener('click', abrirPagamentoModal);
}
function cartRowHtml(c){
  return `<div class="cart-item" data-key="${c.key}">
...
function renderAllCarts(){
  $all('.cart-items-target, #pdv-desktop-cart-items').forEach(box=>{
    if(!Store.cart.length){ box.innerHTML = `<div class="empty-state"><div class="ic">🛒</div>Carrinho vazio.</div>`; return; }
    box.innerHTML = Store.cart.map(cartRowHtml).join('');
    $all('.cart-item', box).forEach(row=>{
      ...
      row.querySelector('[data-act="inc"]').addEventListener('click', ()=>{ item.qtd = +(item.qtd+step).toFixed(3); renderAllCarts(); });
      row.querySelector('[data-act="dec"]').addEventListener('click', ()=>{ item.qtd = Math.max(step, +(item.qtd-step).toFixed(3)); renderAllCarts(); });
      row.querySelector('[data-act="rm"]').addEventListener('click', ()=>{ Store.cart = Store.cart.filter(c=>c.key!==key); renderAllCarts(); });
    });
  });
  $all('.js-cart-total').forEach(e=> e.textContent = fmtBRL(cartTotal()));
  $all('.js-cart-finalizar').forEach(b=> b.disabled = !Store.cart.length);
  renderCartBadge();
}
$('#btn-ver-carrinho').addEventListener('click', renderCarrinhoModal);
$('#pdv-desktop-finalizar').addEventListener('click', abrirPagamentoModal);
...
function abrirPagamentoModal(){
  ...
  $all('.pay-opt').forEach(opt=> opt.addEventListener('click', ()=>{
    ...
    pagamentoSelecionado = opt.dataset.pay; renderPayExtra(total);
  }));
  renderPayExtra(total);
  $('#btn-confirmar-pagamento').addEventListener('click', ()=>confirmarPagamento(total));
}
function buildClienteOptionsHtml(selectedId){
  return `<option value="">Selecione…</option>` +
    Store.clientes.map(c=>`<option value="${c.id}" ${c.id===selectedId?'selected':''}>${c.nome}</option>`).join('') +
    `<option value="__novo">+ Novo cliente</option>`;
}
function renderPayExtra(total){
  const box = $('#pay-extra');
  if(pagamentoSelecionado==='dinheiro'){
    ...
    box.innerHTML = `<div class="field"><label>Cliente *</label><select id="pay-cliente">${buildClienteOptionsHtml()}</select></div>`;
    $('#pay-cliente').addEventListener('change', (e)=>{
      if(e.target.value==='__novo'){
        const nome = prompt('Nome do cliente:');
        if(nome) criarClienteRapido(nome);
        else e.target.value='';
      }
    });
  } else box.innerHTML = '';
...
async function criarClienteRapido(nome){
  try{
    ...
    // Atualização otimista: não espera o listener em tempo real do Firestore
    // para já mostrar o cliente novo selecionado no campo.
    if(!Store.clientes.find(c=>c.id===ref.id)) Store.clientes.push({id:ref.id, nome, telefone:'', fiado:0});
    const sel = document.getElementById('pay-cliente');
    if(sel){ sel.innerHTML = buildClienteOptionsHtml(ref.id); }
    toast('Cliente "'+nome+'" criado e selecionado ✓');
  }catch(err){ toast('Erro ao criar cliente: '+err.message, 'err'); }
}
...
function renderRelatorio(tipo){
  ...
  if(tipo==='fiado'){ renderRelatorioFiado(box); return; }
...
/* ============================================================
   FIADO POR CLIENTE — quem me deve, quanto, e as compras detalhadas
   ============================================================ */
function renderRelatorioFiado(box){
  const porCli = {};
  Store.despesas.filter(d=>d.tipo==='receber').forEach(d=>{
    const key = d.clienteId || '__sem_cliente__';
    if(!porCli[key]){
      const cli = Store.clientes.find(c=>c.id===d.clienteId);
      porCli[key] = { clienteId:d.clienteId, nome: cli? cli.nome : '(cliente removido)', aberto:0, qtdAberto:0, qtdTotal:0 };
    }
    porCli[key].qtdTotal++;
    if(d.status==='pendente'){ porCli[key].aberto += Number(d.valor); porCli[key].qtdAberto++; }
  });
  const arr = Object.values(porCli).sort((a,b)=> b.aberto - a.aberto);
  ultimoRelatorio = { titulo:'Fiado por cliente', headers:['Cliente','Compras em aberto','Total devido'], rows: arr.map(x=>[x.nome, x.qtdAberto, fmtBRL(x.aberto)]) };

  if(!arr.length){
    box.innerHTML = `<h3 style="margin-bottom:10px">Fiado por cliente</h3><div class="empty-state"><div class="ic">📝</div>Nenhuma venda fiado registrada ainda.</div>`;
    return;
  }
  const totalGeralAberto = arr.reduce((s,x)=>s+x.aberto,0);
  box.innerHTML = `<h3 style="margin-bottom:4px">Fiado por cliente</h3>
    <p class="text-sm text-dim mb-0" style="margin-bottom:12px">Total em aberto de todos os clientes: <b class="mono" style="color:var(--danger)">${fmtBRL(totalGeralAberto)}</b> · toque num cliente para ver o detalhe e emitir o PDF de cobrança.</p>` +
    arr.map(x=>`
    <div class="list-row" data-cliente="${x.clienteId||''}">
      <div class="thumb">${x.aberto>0?'🔴':'🟢'}</div>
      <div class="info"><div class="t1">${x.nome}</div><div class="t2">${x.qtdAberto} compra(s) em aberto · ${x.qtdTotal} no total</div></div>
      <div class="right"><div class="price mono" style="color:${x.aberto>0?'var(--danger)':'var(--ok)'}">${fmtBRL(x.aberto)}</div><div class="text-sm text-dim">devendo</div></div>
    </div>`).join('');
  $all('.list-row', box).forEach(row=> row.addEventListener('click', ()=> abrirDetalheFiadoCliente(row.dataset.cliente)));
}

function abrirDetalheFiadoCliente(clienteId){
  const cli = Store.clientes.find(c=>c.id===clienteId);
  const nome = cli? cli.nome : '(cliente removido)';
  const compras = Store.despesas.filter(d=>d.tipo==='receber' && (d.clienteId||'')===(clienteId||'')).sort((a,b)=> new Date(b.data)-new Date(a.data));
  const totalAberto = compras.filter(c=>c.status==='pendente').reduce((s,c)=>s+Number(c.valor),0);
  const html = `
    <div class="modal-head"><h3>${nome}</h3><button class="btn-icon" onclick="closeModal()">✕</button></div>
    <div class="card mb-0" style="padding:12px 14px"><span class="text-sm text-dim">Total em aberto:</span> <b class="mono" style="color:${totalAberto>0?'var(--danger)':'var(--ok)'}">${fmtBRL(totalAberto)}</b></div>
    <div class="perf"></div>
    <div id="fiado-detalhe-list">${comprasFiadoHtml(compras)}</div>
    <div class="fw-grid mt-3">
      <button class="btn btn-outline" id="btn-pdf-fiado-cliente">📄 Exportar PDF</button>
      <button class="btn btn-primary" id="btn-quitar-tudo" ${totalAberto<=0?'disabled':''}>Quitar tudo</button>
    </div>`;
  openModal(html, {center:true});
  bindComprasFiadoEvents(clienteId);
  $('#btn-quitar-tudo').addEventListener('click', async ()=>{
    const pendentes = compras.filter(c=>c.status==='pendente');
    if(!pendentes.length) return;
    if(!confirm(`Marcar todas as ${pendentes.length} compra(s) pendente(s) de ${nome} como pagas?`)) return;
    const batch = db.batch();
    pendentes.forEach(c=> batch.update(db.collection('despesas').doc(c.id), {status:'pago'}));
    await batch.commit();
    toast('Compras quitadas!'); closeModal();
  });
  $('#btn-pdf-fiado-cliente').addEventListener('click', ()=> exportarPdfFiadoCliente(nome, compras, totalAberto));
}
function comprasFiadoHtml(compras){
  if(!compras.length) return `<p class="text-dim text-sm">Nenhuma compra fiado registrada.</p>`;
  return compras.map(c=>`
    <div class="list-row" style="cursor:${c.status==='pendente'?'pointer':'default'}" ${c.status==='pendente'?`data-quitar="${c.id}"`:''}>
      <div class="thumb">${c.status==='pendente'?'🕓':'✅'}</div>
      <div class="info"><div class="t1">${c.descricao}</div><div class="t2">${fmtDateTime(c.data)}${c.status==='pendente'?' · toque para dar baixa':''}</div></div>
      <div class="right"><span class="badge ${c.status==='pago'?'green':'yellow'}">${c.status}</span><div class="mono text-sm">${fmtBRL(c.valor)}</div></div>
    </div>`).join('');
}
function bindComprasFiadoEvents(clienteId){
  $all('[data-quitar]', $('#fiado-detalhe-list')).forEach(row=> row.addEventListener('click', async ()=>{
    if(!confirm('Marcar esta compra como paga/recebida?')) return;
    await db.collection('despesas').doc(row.dataset.quitar).update({status:'pago'});
    toast('Baixado!');
    closeModal(); abrirDetalheFiadoCliente(clienteId);
  }));
}
function exportarPdfFiadoCliente(nome, compras, totalAberto){
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF();
  doc.setFontSize(15); doc.text('Extrato de fiado — '+nome, 14, 18);
  doc.setFontSize(9); doc.text((Store.config.nomeLoja||'MercadoPDV')+' · gerado em '+fmtDateTime(new Date()), 14, 25);
  const rows = compras.map(c=>[fmtDate(c.data), c.descricao, c.status==='pago'?'Pago':'Em aberto', fmtBRL(c.valor)]);
  doc.autoTable({ head:[['Data','Descrição','Status','Valor']], body:rows, startY:32, styles:{fontSize:9} });
  const finalY = (doc.lastAutoTable && doc.lastAutoTable.finalY) || 40;
  doc.setFontSize(12); doc.setFont(undefined,'bold');
  doc.text('Total em aberto: '+fmtBRL(totalAberto), 14, finalY+12);
  doc.save('fiado_'+nome.replace(/\s+/g,'_')+'_'+todayStr()+'.pdf');
}
